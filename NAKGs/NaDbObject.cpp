// GLObject.cpp: implementation of the NaDbObject class.
//
//////////////////////////////////////////////////////////////////////
...
NaDbObject::NaDbObject()
{
    ...
    //matl
    itsMaterial = NaDbMaterial(ALUMINIUM);
...
DbObjectType NaDbObject::GLObjType() const
{
    return glObjType;
}

bool NaDbObject::IsOfType(const DbObjectType& type) const
{
    DbObjectType typ = type;
    bool b = (glObjType == typ);
    return b;
}

void NaDbObject::SetDisplayMode(const GsDisplayMode& mode)
{
    displayMode = mode;
    DefineDisplay();
}

GsDisplayMode NaDbObject::GetDisplayMode() const
{
    return displayMode;
}

void NaDbObject::SetMaterial(const GsMaterialName& matName)
{
    SetMaterial(NaDbMaterial(matName));
}

void NaDbObject::SetMaterial(const NaDbMaterial& mat)
{
    itsMaterial = mat;
    ambiMat[0] = itsMaterial.GetAmbient().GetRed();
    ambiMat[1] = itsMaterial.GetAmbient().GetGreen();
...
NaDbMaterial NaDbObject::GetMaterial() const
{
    return itsMaterial;
}

void NaDbObject::ApplyMaterial()
{
...
void NaDbObject::SetColor(const GLubyte& red, const GLubyte& green, const GLubyte& blue)
{
    itsRed = red;
    itsGreen = green;
    itsBlue = blue;
...
void NaDbObject::GetColor(GLubyte* colVect) const
{
...
void NaDbObject::SetBoundingBox(const NaGeBoundingBox& B)
{
    itsBox = B;
}

NaGeBoundingBox NaDbObject::GetBoundingBox() const
{
    return itsBox;
}
